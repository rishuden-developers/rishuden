...
class CreditResultPage extends StatefulWidget {
  ...
  final String?
  rankingType; // ランキングの種類 ('easiness', 'satisfaction', 'faculty', 'nume')
...
class _CreditResultPageState extends State<CreditResultPage> {
  ...
  void _filterAndSortLectures() {
    ...
    if (widget.searchQuery != null && widget.searchQuery!.isNotEmpty) {
      filteredList =
          filteredList
              .where(
                (lecture) =>
                    lecture.name.toLowerCase().contains(
                      widget.searchQuery!.toLowerCase(),
                    ) ||
                    lecture.teacher.toLowerCase().contains(
                      widget.searchQuery!.toLowerCase(),
                    ),
              )
              .toList();
    ...
    if (widget.filterFaculty != null && widget.filterFaculty!.isNotEmpty) {
      filteredList =
          filteredList
              .where((lecture) => lecture.faculty == widget.filterFaculty)
              .toList();
    }
    ...
    switch (widget.rankingType) {
      ...
      case 'nume': // 沼単ランキング
        _pageTitle = '沼単ランキング';
        filteredList.sort(
          (a, b) => a.easiness.compareTo(b.easiness),
        ); // 楽単度が低い順
        break;
      default:
        _pageTitle =
                  ...
                  _lectureResults.isEmpty
                      ...
                      : ListView.builder(
                        padding: EdgeInsets.only(
                          top:
                              AppBar().preferredSize.height + 20, // AppBarの下の余白
