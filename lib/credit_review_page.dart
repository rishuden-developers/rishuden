...
class _CreditReviewPageState extends State<CreditReviewPage> {
  ...
  final TextEditingController _teacherFeatureController = TextEditingController(); // 教員の特徴
  final TextEditingController _commentController = TextEditingController(); // おすすめコメント
  ...
  final List<String> _availableTags = [
    '#楽単', '#テストなし', '#出席ゆるい', '#課題少なめ', '#レポートのみ',
    '#グループワーク', '#アクティブラーニング', '#過去問必須', '#教員が優しい',
    '#教員が厳しい', '#教員が面白い', '#オンライン完結', '#対面推奨',
    '#板書が丁寧', '#スライド分かりやすい', '#ディスカッション多め',
  ];
        ...
        Container(
          ...
          child: DropdownButtonHideUnderline(
            child: DropdownButton<T>(
              ...
              hint: const Text('選択してください', style: TextStyle(color: Colors.grey)),
              onChanged: onChanged,
              items: items,
            ),
          ),
        ...
        TextFormField(
          ...
          decoration: InputDecoration(
            ...
            border: OutlineInputBorder(
              borderRadius: BorderRadius.circular(8),
              borderSide: BorderSide(color: Colors.orangeAccent[100]!, width: 1.5),
            ),
      ...
      children: [
        ...
        Wrap(
          ...
          children: _availableTags.map((tag) {
            ...
            return FilterChip(
              label: Text(tag, style: TextStyle(color: isSelected ? Colors.white : Colors.brown[700])),
              selected: isSelected,
              onSelected: (selected) {
                setState(() {
                  if (selected) {
              ...
              shape: RoundedRectangleBorder(
                ...
                side: BorderSide(color: Colors.orangeAccent[100]!, width: 1.0),
              ),
  ...
  void _submitReview() {
    if (_formKey.currentState!.validate()) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('レビューを投稿中...')),
      );
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      extendBody: true,
      appBar: AppBar(
        title: Text(widget.selectedLectureName != null
      ...
      bottomNavigationBar: CommonBottomNavigation(
        ...
        onParkTap: () {
          Navigator.pushReplacement(
            context,
            PageRouteBuilder(
              pageBuilder: (context, animation, secondaryAnimation) => const ParkPage(),
        ...
        onTimetableTap: () {
          Navigator.pushReplacement(
            context,
            PageRouteBuilder(
              pageBuilder: (context, animation, secondaryAnimation) => const TimeSchedulePage(),
              transitionDuration: Duration.zero,
              reverseTransitionDuration: Duration.zero,
            ),
          );
        ...
        onRankingTap: () {
          Navigator.pushReplacement(
            context,
            PageRouteBuilder(
              pageBuilder: (context, animation, secondaryAnimation) => const RankingPage(),
              transitionDuration: Duration.zero,
              reverseTransitionDuration: Duration.zero,
            ),
          );
        ...
        onItemTap: () {
          Navigator.pushReplacement(
            context,
            PageRouteBuilder(
              pageBuilder: (context, animation, secondaryAnimation) => const ItemPage(),
              transitionDuration: Duration.zero,
              reverseTransitionDuration: Duration.zero,
            ),
          );
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      _buildDropdownSection<ExamType>(
                        ...
                        items: const [
                          DropdownMenuItem(value: ExamType.report, child: Text('レポート')),
                          DropdownMenuItem(value: ExamType.written, child: Text('筆記試験')),
                          DropdownMenuItem(value: ExamType.attendanceBased, child: Text('出席点のみ')),
                          DropdownMenuItem(value: ExamType.none, child: Text('試験なし')),
                          DropdownMenuItem(value: ExamType.other, child: Text('その他')),
                        ],
                      ),
                      _buildDropdownSection<AttendanceStrictness>(
                        ...
                        items: const [
                          DropdownMenuItem(value: AttendanceStrictness.flexible, child: Text('自由')),
                          DropdownMenuItem(value: AttendanceStrictness.everyTimeRollCall, child: Text('毎回点呼')),
                          DropdownMenuItem(value: AttendanceStrictness.attendancePoints, child: Text('出席点あり')),
                          DropdownMenuItem(value: AttendanceStrictness.noAttendance, child: Text('出席なし')),
                        ],
                      ),
                      _buildDropdownSection<LectureFormat>(
                        ...
                        items: const [
                          DropdownMenuItem(value: LectureFormat.faceToFace, child: Text('対面')),
                          DropdownMenuItem(value: LectureFormat.onDemand, child: Text('オンデマンド')),
                          DropdownMenuItem(value: LectureFormat.zoom, child: Text('Zoom（オンライン）')),
                          DropdownMenuItem(value: LectureFormat.other, child: Text('その他')),
                        ],
                      ),
                      _buildTextField(
                      ...
                      ElevatedButton(
                        ...
                        style: ElevatedButton.styleFrom(
                          ...
                          shape: RoundedRectangleBorder(
                            ...
                            side: BorderSide(color: Colors.amberAccent[100]!, width: 2),
                          ),
                          elevation: 6,
