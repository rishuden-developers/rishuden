...
class _CreditReviewPageState extends State<CreditReviewPage> {
  ...
  void initState() {
    super.initState();
    _allReviews = _generateDummyReviewsForLecture(widget.lectureName, widget.teacherName);
    _applyFilters();
  }

  List<LectureReview> _generateDummyReviewsForLecture(String lectureName, String teacherName) {
    ...
    return [
      LectureReview(
        ...
        comment: 'この講義は最高でした！先生の説明も分かりやすく、内容も興味深かったです。レポートは大変でしたが、得るものが多かったです。',
    ...
    ].where((review) => review.lectureName == lectureName && review.teacherName == teacherName).toList();
  }
  ...
  void _applyFilters() {
    _filteredReviews = _allReviews.where((review) {
      ...
      if (_selectedAttendanceFilter != null) {
        matches = matches && review.attendanceStrictness == _selectedAttendanceFilter;
      }
  ...
  double get _averageOverallSatisfaction {
    ...
    return _allReviews.map((r) => r.overallSatisfaction).reduce((a, b) => a + b) / _allReviews.length;
  }

  double get _averageEasiness {
    if (_allReviews.isEmpty) return 0.0;
    return _allReviews.map((r) => r.easiness).reduce((a, b) => a + b) / _allReviews.length;
  }
                  ...
                  children: [
                    ...
                    if (_selectedFormatFilter != null ||
                      ...
                      ElevatedButton.icon(
                        ...
                        icon: const Icon(Icons.clear_all, color: Colors.indigo),
                        label: const Text(
                          'フィルターをリセット',
                          style: TextStyle(
                            fontSize: 16,
                        ...
                        style: ElevatedButton.styleFrom(
                          ...
                          shape: RoundedRectangleBorder(
                            ...
                            side: BorderSide(color: Colors.indigo[200]!, width: 1),
                          ),
                          elevation: 2,
                        ),
                      ),
                                ...
                                child: Padding(
                                  ...
                                  child: Column(
                                    ...
                                    children: [
                                      ...
                                      Text('形式: ${_formatEnum(review.lectureFormat)}',
                                          style: const TextStyle(color: Colors.black87)),
                                      Text('出席: ${_formatEnum(review.attendanceStrictness)}',
                                          style: const TextStyle(color: Colors.black87)),
                                      Text('試験: ${_formatEnum(review.examType)}',
                                          style: const TextStyle(color: Colors.black87)),
                                      Text('教員特徴: ${review.teacherFeature}',
                                          style: const TextStyle(color: Colors.black87)),
                                      const SizedBox(height: 8),
                                      Text('コメント: ${review.comment}',
                                          style: const TextStyle(color: Colors.black87)),
                                      const SizedBox(height: 8),
                                      Wrap(
                                        spacing: 6.0,
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            Row(
              children: [
                const Icon(Icons.sentiment_satisfied, color: Colors.lightGreen, size: 24),
                const SizedBox(width: 8),
                Text(
                  '楽単度: ${_averageEasiness.toStringAsFixed(1)}',
                  style: const TextStyle(
                      fontSize: 18, fontWeight: FontWeight.bold, color: Colors.lightGreen),
                ),
              ],
            ),
          ],
    ...
    return ElevatedButton.icon(
      onPressed: () {
        Navigator.push(
          context,
          MaterialPageRoute(
            builder: (context) => CreditInputPage(
  ...
  Widget _buildFilterDropdown<T>(
      String hintText, T? selectedValue, List<T> items, ValueChanged<T?> onChanged) {
    return Container(
